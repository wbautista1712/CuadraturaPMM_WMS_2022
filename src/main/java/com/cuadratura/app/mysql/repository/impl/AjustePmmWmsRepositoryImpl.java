...
public class AjustePmmWmsRepositoryImpl implements AjustePmmWmsRepositoryCustom {
	...
	public List<Object[]> getAllBolsaSdi() {
		...
		String sql = "	SELECT ATE.TRANS_USER, ATE.TRANS_BATCH_DATE, ATE.TRANS_SOURCE, ATE.TRANS_AUDITED,  ATE.INV_MRPT_CODE,			"
				...
				"		(select pmm.f_obtenerCodigoSecuencial(TI.tipoInventario,(CASE WHEN AJ.sugerenciaAjuste>0 THEN 63 ELSE 64 END))) as INV_DRPT_CODE,  		"
				+
				...
				"	FROM pmm.ajuste_pmm_wms AJ 		"
				+ "	INNER JOIN pmm.tbl_pmm_wms TBL ON AJ.id_tbl_pmm_wms=TBL.id_tbl_pmm_wms 			"
				+ "	INNER JOIN pmm.cruce_pmm_wms Cru ON TBL.idCruce_pmm_wms=Cru.idCruce_pmm_wms 		"
				+ "	INNER JOIN pmm.carga_wms WMS ON Cru.idCarga_WMS=WMS.idCarga_WMS 		"
				+ "	INNER JOIN pmm.m_orgmstee CD ON WMS.org_name_short=CD.org_name_short 		"
				+ "	INNER JOIN pmm.m_prdmstee PROD ON TBL.mat_prd_lvl_child=PROD.prd_lvl_child 			" +

				"	INNER JOIN pmm.m_tipo_inventario TI ON AJ.id_tipo_inventario=TI.id_tipo_inventario 			"
				...
				"		(select pmm.f_obtener_cod_BolsaDiscrepancia(TI.tipoInventario,(CASE WHEN AJ.sugerenciaAjuste>0 THEN 64 ELSE 63 END)))  AS TRANS_TYPE_CODE,       	"
				+

				"		(CASE WHEN AJ.sugerenciaAjuste>0 THEN 64 ELSE 63 END) AS TRANS_TRN_CODE,		" +

				"		(select pmm.f_obtener_codIngreso_Salida(TI.tipoInventario,(CASE WHEN AJ.sugerenciaAjuste>0 THEN 64 ELSE 63 END))) as INV_DRPT_CODE,		"
				+
	...
	public void updateAjustePmmWms(Integer idAjustePMMWMS) {
		...
		String query = "UPDATE pmm.ajuste_pmm_wms " + "SET estado=:estado "
